var focus = {focus:1};
FWMultiGrid =  {
	HandleDecimalMask:function(system_module_name,db_field,decimals)
	{
	 $("#m_"+system_module_name+" #"+db_field).inputmask({'mask':"9{0,5}.9{0,"+decimals+"}", greedy: false,rightAlign: true});
	},
	...
	row_obj['group_int_code']=0;
	 ...
	 last_row_id=last_id+1;
						...
						$(input_id).append("<tr style='height:20px;' id='"+last_row_id+"' onDblclick=\"FWMultiGrid.EditGridRow('"+system_module_name+"','"+group_table_name+"',"+last_row_id+");\"></tr>");
	...
	try {
			...
			$.each(grp_ele_arr, function (ek,ev) {			
				...
				var accept_negative_number=ev.accept_negative_number;
				var number_range_from=ev.number_range_from;
				var number_range_to=ev.number_range_to;
				var add_in_popup=ev.add_in_popup;
				var decimals=ev.decimals;
				...
				var range_on_blur="";
				if(number_range_to != 0 && number_range_from != 0)
				{
					range_on_blur= "HandleNumberFromToRangeValidation('"+system_module_name+"','"+system_name+"','"+number_range_from+"','"+number_range_to+"');";
				}
				if(!empty(on_exit))
				{
					on_exit_event=" onBlur=\""+on_exit+range_on_blur+"\"";
				}
				else if(!empty(range_on_blur))
				{
					on_exit_event=" onBlur=\""+range_on_blur+"\"";
				}
				...
				if(data_type=='Number')
				{
					if(accept_negative_number='No')
						accept_no= "onkeypress='return acceptNumbersOnlyForModuleForDec($(this).val(),event,"+decimals+")'";
					else
						accept_no= "onkeypress='return acceptNumbersWithNegativeForModuleForDec($(this).val(),event,"+decimals+")'";
				...
				if(data_type=='Reference Data')
				{
					...
					var ref_module_name=ev.ref_module_name;
					...
					{
						group_html+="<td "+td_display+">";
						// group_html+="		<div class='input-group' style='margin-top:6px; width:"+grid_width+"px;'>";
						group_html+="		<input  class='form-control input-sm'   id='"+db_field+"'   type='hidden'  name='"+ref_module_code+"'  value='"+td_val+"' />";
						group_html+="		<div class='input-group' style='width:"+grid_width+"px;'>";
						...
						group_html+="			<input class='form-control input-sm' style='border-bottom-left-radius: 4px; border-top-left-radius: 4px;' placeholder='"+name+"'  value='"+span_val+"'  on_exit=\""+on_exit+"\"   "+on_enter_event+"   "+read_only+"  "+tab_index+" id='txt_ref_schema_"+db_field+"'  type='text' style=' ' value=''>";
						group_html+="	 	<span class='input-group-addon'>";
						group_html+="		<i style='cursor:pointer;' class='fa fa-search' title='Search in "+ref_module_dis_name+"' onclick=\" POPUP.OpenFWPopup('"+ref_module_table+"','"+system_module_name+"' ,'"+system_name+"');\" >";
						group_html+="		</i></span>";
						group_html+="<script>POPUP.AssignJqueryAutoCompleteNew('"+ref_module_table+"','"+system_module_name+"' ,'"+system_name+"','','"+ref_module_code+"');</script>";
							
						if(add_in_popup==1)
						{
							group_html+="	  <span class='input-group-addon'>";
							group_html+="<i style='cursor:pointer;' class='fa fa-plus' title='Add New "+ref_module_dis_name+"' onclick=\" AssignPopupModuleDetails('"+ref_module_table+"','"+ref_module_name+"','"+ref_module_dis_name+"','"+ref_module_code+"','"+system_module_name+"')\">";
							group_html+="</i></span>";
						}
				...
				else if(data_type=='List Of Values')
				{
					...
					group_html+="<td "+td_display+">";
					// group_html+="		<select id='"+db_field+"' "+disabled+"  "+tab_index+"  "+on_change_event+" class='form-control input-sm' style='width:"+grid_width+"px;'>";
					group_html+="		<select id='"+db_field+"' "+disabled+"  "+tab_index+"  "+on_change_event+" class='form-control input-sm' style='margin-top:0px; width:"+grid_width+"px;'>";
				...
				else if(data_type=='Date')
				{
					...
					group_html+="<td "+td_display+">";
					// group_html+="<div data-date-viewmode='years' data-date-format='dd/mm/yyyy' class='input-group'  style='margin-top:6px; width:"+grid_width+"px;'>";	
					group_html+="<div data-date-viewmode='years' data-date-format='dd/mm/yyyy' class='input-group'  style='margin-top:0px; width:"+grid_width+"px;'>";	
					...
					group_html+="<span class='input-group-addon'><i class='icon-calender' style='cursor:pointer; color:#333333;' onclick=\"all_obj.CallTarkaDatePickerNew('"+system_module_name+"','"+db_field+"')\" ></i></span>";	
				...
				else if(data_type=='DateTime')
				{
					...
					group_html+="<td "+td_display+">";
					// group_html+="<div data-date-viewmode='years' data-date-format='dd/mm/yyyy' class='input-icon right'  style='margin-top:6px; width:"+grid_width+"px;'>";	
					group_html+="<div data-date-viewmode='years' data-date-format='dd/mm/yyyy' class='input-icon right'  style='margin-top:0px; width:"+grid_width+"px;'>";	
				...
				{ 
					group_html+="<td "+td_display+">";
					// group_html+="		<input type='text' "+on_exit_event+" "+on_enter_event+" placeholder='"+name+"' maxlength='20' id='"+db_field+"' value='"+td_val+"' style='margin-top:6px; "+text_align+"; width:"+grid_width+"px;'  "+tab_index+" "+read_only+" class='form-control input-sm' "+accept_no+">";
					group_html+="		<input type='text' "+on_exit_event+" "+on_enter_event+" placeholder='"+name+"' maxlength='20' id='"+db_field+"' value='"+td_val+"' style='margin-top:0px; "+text_align+"; width:"+grid_width+"px;'  "+tab_index+" "+read_only+" class='form-control input-sm' "+accept_no+">";
					group_html+="  </td>";
				    // <script>FWMultiGrid.HandleDecimalMask('"+system_module_name+"','"+db_field+"','"+decimals+"')</script>
				}
				...
				{ 
					...
					group_html+="<td "+td_display+">";
					group_html+="	 <textarea rows='"+text_area_rows+"' cols='1'   style='"+text_align+"; margin-top:0px; width:"+grid_width+"px;'   id='"+db_field+"'  class='form-control input-sm'> "+td_val+"</textarea> ";
					group_html+="</td>";
				}
				...
				{ 
					...
					if(type=='Edit')
					{
						...
						group_html+="<div style='width:"+grid_width+"px;'><input type='file'  id='"+db_field+"' style='float:left; width: 100%;' data-att-state='"+data_att_state+"' onchange=\"all_obj.ReadAttachment('"+system_module_name+"','"+db_field+"')\" data-file-src='"+data_file_src+"'  data-file-name='"+data_file_name+"' type='text'   />";
						...
						group_html+="	<button type='button' class='btn btn-danger btn-circle'><i class='ti-trash'></i></button>";
						group_html+=" </span>";
						group_html+=" </div>";
					}
					else
					{
						group_html+="<div style='width:"+grid_width+"px;'><input type='file'  id='"+db_field+"' style='float:left; width: "+grid_width+"px;' data-att-state='' onchange=\"all_obj.ReadAttachment('"+system_module_name+"','"+db_field+"')\" data-file-src=''  data-file-name='' type='text'   />";
				...
				{	
					...
					group_html+="<td "+td_display+">";
					group_html+="<div id='"+db_field+"' style='margin-top:0px; width:"+grid_width+"px; text-align: center;'>";
					...
					group_html+="		<div>";
					group_html+="			<span class='btn default btn-sm' style='padding:2px; font-size:12px;'>";
				...
				else if(data_type=="Checkbox")
				{
					...
					group_html+="<td "+td_display+">";
					group_html+="	  <input type='checkbox' onclick=\""+on_enter+"\" "+checked+" id='"+db_field+"' "+disabled+"  "+tab_index+" class='input-sm' style='width:40px;height:15px;margin-top:6px;'>";
					group_html+="</td>";
				}
				else 
				{
					...
					group_html+="<td "+td_display+">";
					// group_html+="		<input type='text' "+user_read_only+"  "+on_exit_event+"  "+on_enter_event+" placeholder='"+name+"' id='"+db_field+"' value=\""+EscapeDoubleQuote(td_val)+"\" style='"+text_align+"; margin-top:6px; width:"+grid_width+"px;' "+read_only+" "+tab_index+"  class='form-control input-sm' "+accept_no+">";
					group_html+="		<input type='text' "+user_read_only+"  "+on_exit_event+"  "+on_enter_event+" placeholder='"+name+"' id='"+db_field+"' value=\""+EscapeDoubleQuote(td_val)+"\" style='"+text_align+"; margin-top:0px; width:"+grid_width+"px;' "+read_only+" "+tab_index+"  class='form-control input-sm' "+accept_no+">";
			...
			{
				child_group_html+="		<span class='btn btn-sm dark tooltip-primary' onkeypress='return keyHandler(event,this);'  tabindex=0 data-placement='bottom' onclick=\"FWMultiGrid.OpenThirdLevelPopup('"+system_module_name+"','"+group_table_name+"','"+tr_id+"',"+group_int_code+");\" title='Third Level' data-toggle='tooltip' data-original-title='Third Level' style='padding: 0px 0px;'>";
				child_group_html+="			<button type='button' class='btn btn-dribbble btn-circle'><i class='fa fa-sitemap'></i></button>";
				child_group_html+="		</span>";
			}
			
			group_html+="<td style='width: 200px;text-align:center;' id='actions'>";
			group_html+="		<span class='btn btn-sm green tooltip-primary' onkeypress='return keyHandler(event,this);' tabindex=0 data-placement='bottom' onclick=\"return FWMultiGrid.SaveTableRecord('"+system_module_name+"','"+group_table_name+"','"+tr_id+"','Grid');\" title='Save' data-toggle='tooltip' data-original-title='Save' style='padding: 0px 0px;'>";
			group_html+="			<button type='button' class='btn btn-success btn-circle'><i class='ti-check'></i></button>";
			group_html+="		</span>";
			group_html+="		<span class='btn btn-sm default tooltip-primary'  onkeypress='return keyHandler(event,this);' tabindex=0 data-placement='bottom' id='row_cancel' data-cancel='"+cancel_row_obj+"' "+cancel_function+" data-row-type='"+row_type+"' title='"+hover+"' data-toggle='tooltip' data-original-title='"+hover+"' style='padding: 0px 0px;'>";
			group_html+="			<button type='button' class='btn btn-warning btn-circle'><i class='fa fa-times'></i> </button>";
			...
			if(parent_group==0)
			{
				if(is_edit_pop_up==1)
				{
					group_html+="&nbsp;<span class='btn btn-sm yellow tooltip-primary' onkeypress='return keyHandler(event,this);'  tabindex=0 data-placement='bottom' onClick=\"FWMultiGrid.EditGroupRowinPopup('"+system_module_name+"','"+group_table_name+"',"+tr_id+",'"+pop_up+"')\" title='Edit in Popup' data-toggle='tooltip' data-original-title='Edit in Popup' style='padding: 0px 0px;'>"; 
					group_html+="<button type='button' class='btn btn-primary btn-circle'><i class='fa fa-external-link'></i> </button></span>";
			...
			if($(select_id).length>0)
			{
				if(is_delete_row==1)
				{
					group_html+="&nbsp;<span class='btn btn-sm red tooltip-primary' onkeypress='return keyHandler(event,this);' tabindex=0 data-placement='bottom' onClick=\"FWMultiGrid.DeleteGroupRow('"+system_module_name+"','"+group_table_name+"',"+tr_id+")\" title='Delete' data-toggle='tooltip' data-original-title='Delete' style='padding: 0px 0px;'>"; 
					group_html+="<button type='button' class='btn btn-danger btn-circle'><i class='ti-trash'></i></button></span>";
				}
			}
			...
			if(type=='Add')
			{	
				var add_html="<tr id='"+tr_id+"' style='height:20px;' class='' onDblclick=\"FWMultiGrid.EditGridRow('"+system_module_name+"','"+group_table_name+"',"+tr_id+");\">";
				add_html+=group_html;
				add_html+="</tr>";
			...
			$("#m_"+system_module_name+" #"+group_table_name+" input").keyup(function(e){
				var  trid = $(this).closest('tr').attr('id'); // table row ID 
				if(e.keyCode == 119)
				{
					$(this).blur();
					FWMultiGrid.SaveTableRecord(system_module_name,group_table_name,trid,'Add');
				}
			}); 
			
			$("#multi_grid_search_"+group_table_name).keypress(function (e) {
				FWMultiGrid.CallMultiGridSearch(system_module_name,group_table_name);
			});
	...
	CallMultiGridSearch : function(system_module_name,group_table_name)
	{
		var value = $("#multi_grid_search_"+group_table_name).val();
		$("#m_"+system_module_name+" #"+group_table_name+" tbody tr").filter(function() {
	...
	SaveTableRecord : function (system_module_name,table_id,tr_id,save_type) {
		...
		var tr_data_id=$("#m_"+system_module_name+" #"+group_name+" tbody tr[id='"+tr_id+"']");
			$.each(grp_ele_arr, function (ek,ev) {			
					...
					else
					{
						...
						var cell_val=$(tr_data_id).find("#"+db_field).val();
						if(save_type=='Popup')
							cell_val=$(input_id+" #"+db_field).val();
						if(data_type=='Reference Data')
						{
							if(populate_lov==1 && is_ref_module_code_lov==1)
							{
								if(optional=='0' && empty(cell_val))
								{
									...
									$(tr_data_id).find("#"+db_field).focus();
									return false;
								}
							...
							else
							{
								if(optional=='0' && empty(cell_val))
								{
									is_error=false;
									ShowAlertMsg("Please Enter "+name);
										$(tr_data_id).find("#txt_ref_schema_"+db_field).focus();
									return false;
								}
								var ref_txt_val=$(tr_data_id).find("#txt_ref_schema_"+db_field).val();
								if(save_type=='Popup')
									ref_txt_val=$("#txt_ref_schema_"+db_field).val();
	...
	FormGridRowFromInputToTableRow : function (system_module_name,group_table_name,row_obj,tr_id,new_row_required,fw_edit_mode) {
		...
		try {
			...
			$.each(grp_ele_arr, function (ek,ev) {	
				...
				if(data_type=='Reference Data')
				{
					if(populate_lov==1 && is_ref_module_code_lov==1)
					{
						...
						tr+="<td "+td_display+"><p style='margin-top:0px; width:"+grid_width+"px;'  class='td-txt' title='"+text_val+"'> <input style='width:0px;' type='hidden' id='"+db_field+"_"+tr_id+"' value="+val+"><span id='txt_ref_schema_"+db_field+"_"+tr_id+"'>"+text_val+"</span></p></td>";
				...
				else if(data_type=='Date')
				{
					...
					tr+="<td id='"+db_field+"_"+tr_id+"' class='text-right' "+td_display+"><p  class='td-txt' style='margin-top:0px; width:"+grid_width+"px;' title='"+val+"'>"+val+"</p></td>";
				}
				else if(data_type=='Number')
				{
					if(val == 'null')
				...
				else if(data_type=='Attachment')
				{
					...
					if(empty(val))
						data_file_src="";
						tr+="<td id='"+db_field+"_"+tr_id+"' class='text-right' "+td_display+"><p  data-att-state='"+data_att_state+"' data-file-src='"+data_file_src+"' class='td-txt' style='margin-top:0px; width:"+grid_width+"px;'  data-file-name ='"+attch_name+"' >"+attch_name+"</p></td>";
				}
				else if(data_type=='Image')
				{
					var image_data="";
					...
					tr+="<td id='"+db_field+"_"+tr_id+"' class='text-center' "+td_display+"><p class='td-txt' style='margin-top:0px; width:"+grid_width+"px;'>"+image_tag+"</p></td>";
				}
				else if(data_type=='Checkbox')
				{
					var checked='';
					...
					tr+="<td id='"+db_field+"_"+tr_id+"' class='text-center' "+td_display+"><p  class='td-txt' style='margin-top:0px; width:"+grid_width+"px;'>"+checkbox_html+"</p></td>";
				}
				else if(data_type=='Large Text')
				{
				
					tr+="<td id='"+db_field+"_"+tr_id+"' class='text-left' "+td_display+"><p  class='td-txt' style='margin-top:0px; white-space:pre;width:"+grid_width+"px;' title='"+decodeURIComponent(val)+"'>"+decodeURIComponent(val)+"</p></td>";
				}
				else
				{
				
					tr+="<td id='"+db_field+"_"+tr_id+"' class='text-left' "+td_display+"><p  class='td-txt' style='margin-top:0px; width:"+grid_width+"px;' title='"+decodeURIComponent(val)+"'>"+decodeURIComponent(val)+"</p></td>";
				}
				
			});
			
			var tr_actions="<span class='btn btn-sm blue tooltip-primary' tabindex=0  data-placement='bottom' onClick=\"FWMultiGrid.FormGridEditFields('"+system_module_name+"','"+group_table_name+"',"+tr_id+",'Edit')\" title='Edit' data-toggle='tooltip' data-original-title='Edit' style='padding: 0px 0px;'>"; 
			tr_actions+="<button type='button' class='btn btn-default btn-circle'> <i class='fa fa-edit' style='font-size:12px;'></i></button>";
			tr_actions+="</span>";
			
			if(is_add_row==1)
			{
				//Add New Row                                                                                                            
				tr_actions+="&nbsp;<span class='btn btn-sm purple tooltip-primary'  tabindex=0 data-placement='bottom' onClick=\"FWMultiGrid.FormGridEditFields('"+system_module_name+"','"+group_table_name+"',"+tr_id+",'Add')\" title='Add New' data-toggle='tooltip' data-original-title='Add New ' style='padding: 0px 0px;'>"; 
				tr_actions+="<button type='button' class='btn btn-info btn-circle'>  <i class='fa fa-plus'></i></button></span>";
			}
			if(parent_group==0)
			{
				if(is_edit_pop_up==1)
				{
					//Edit Row in popup
					tr_actions+="&nbsp;<span class='btn btn-sm yellow tooltip-primary' tabindex=0  data-placement='bottom' onClick=\"FWMultiGrid.EditGroupRowinPopup('"+system_module_name+"','"+group_table_name+"',"+tr_id+",'RowPopup')\" title='Edit in Popup' data-toggle='tooltip' data-original-title='Edit in Popup' style='padding: 0px 0px;'>"; 
					tr_actions+="<button type='button' class='btn btn-primary btn-circle'><i class='fa fa-external-link'></i> </button></span>";
			...
			{
				//Delete Row
				tr_actions+="&nbsp;<span class='btn btn-sm red tooltip-primary'  tabindex=0 data-placement='bottom' onClick=\"FWMultiGrid.DeleteGroupRow('"+system_module_name+"','"+group_table_name+"',"+tr_id+")\"  title='Delete' data-toggle='tooltip' data-original-title='Delete' style='padding: 0px 0px;'>"; 
				tr_actions+="<button type='button' class='btn btn-danger btn-circle'><i class='ti-trash'></i></button></span>";
			}
			var child_group_html="";
			if(is_child_exists==1)
			{
				child_group_html+="		<span class='btn btn-sm dark tooltip-primary' tabindex=0  onclick=\"FWMultiGrid.OpenThirdLevelPopup('"+system_module_name+"','"+group_table_name+"','"+tr_id+"',"+group_int_code+");\" data-placement='bottom' title='Third Level' data-toggle='tooltip' data-original-title='Third Level' style='padding: 0px 0px;'>";
				child_group_html+="			<button type='button' class='btn btn-dribbble btn-circle'><i class='fa fa-sitemap'></i></button>";
				child_group_html+="		</span>";
			}
			...
			if(is_copy_row==1)
			{
				tr_actions+="<span class='btn btn-sm green tooltip-primary' tabindex=0  data-placement='bottom' onClick=\"FWMultiGrid.CopyRowData('"+system_module_name+"','"+group_table_name+"',"+tr_id+")\" title='Copy' data-toggle='tooltip' data-original-title='Copy' style='padding: 0px 0px;'>"; 
			...
			tr+="<td id='actions_"+tr_id+"' style='width:200px; text-align:center;'>"+tr_actions+"</td>";
		...
		/*	$("#m_"+system_module_name+" #"+group_table_name +" tbody").tableDnD({
			...
			// var row_sequence = $(group_table_name+"#"+tr_id+" #seq_"+tr_id+" p").html();
			// alert(row_sequence);
			//SAVE ROW DATA TO ENTRY TABLE
			this.SaveRowDataToEntryTable(system_module_name,group_table_name,row_obj,tr_id,new_row_required,fw_edit_mode);
	...
	//SAVE ROW DATA TO ENTRY TABLE
	SaveRowDataToEntryTable : function (system_module_name,group_table_name,row_obj,tr_id,new_row_required,fw_edit_mode)
	{
	
		/* var row_sequence = tr_id;
		
		var paramJSON =  encodeURIComponent(JSON.stringify(row_obj));
		
		var data_str="paramJSON="+paramJSON;
			data_str+="&system_module_name="+system_module_name;
			data_str+="&group_table_name="+group_table_name;
			data_str+="&tr_id="+tr_id;
			data_str+="&new_row_required="+new_row_required;
			data_str+="&fw_edit_mode="+fw_edit_mode;
			data_str+="&row_sequence="+row_sequence;
			
		$.ajax({
			type: "POST",	 
			url: $host_url+"SaveRowDataToEntryTable", 
			async: true,	
			data:data_str,	
			error:AjaxErrorMessage,		
			success:  function SaveRowDataToEntryTableSuccess(responce)
			{
				App.unblockUI();
	...
	EditGroupRowinPopup : function(system_module_name,group_table_name,tr_id,popup_type) 
	{
		...
		try {
			   ...
			   // ShowAlertMsg("Don't have Rights");
			   // return true;  
			...
			footer_row+="<div id='m_"+system_module_name+"_popup_footer'  style='margin:0;'"; 
			footer_row+='	class="form-actions pull-right"> '; 
			footer_row+="	<span class='btn btn-sm btn-info1' onclick=\"FWMultiGrid.SaveTableRecord('"+system_module_name+"','"+group_table_name+"','"+tr_id+"','Popup');\">Save <i class='fa fa-check'></i></span>"; 
			...
			footer_row+="	<input type='hidden' id='pop_up_row_int_code' value='"+tr_id+"' /> <span class='btn btn-sm btn-danger'  onclick=\"$('#myModal').modal('hide');FWMultiGrid.CloseEditGridPopup('"+system_module_name+"','"+group_table_name+"','"+tr_id+"','"+popup_type+"');\" >Cancel <i class='fa fa-times'></i></span></div> "; 
			...
			$("#myModal").on("hidden.bs.modal", function (e) {
				FWMultiGrid.CloseEditGridPopup(system_module_name,group_table_name,tr_id,popup_type);
	...
	//cancel edit grid row
	 CancelEditGridRow : function(system_module_name,group_table_name,tr_id) {
		...
		try {
			...
			if(!empty(row_obj) && row_obj!='undefined')
			{
	...
	AssignGroupDetailsAfterEdit : function (system_module_name,grp_details) {
			...
			{				
				...
				$.each(grp_details, function (gk,gv) 
				{
					...
					//var grp_ele_arr=grp_prop['grp_ele_arr'];
					
					var is_child_exists=grp_prop['is_child_exists'];
					var parent_group=grp_prop['parent_group'];
					var is_add_row=grp_prop['is_add_row'];
					var is_copy_row=grp_prop['is_copy_row'];
					var is_delete_row=grp_prop['is_delete_row'];
					var is_edit_pop_up=grp_prop['is_edit_pop_up'];
					...
					else if(!empty(gv))
					{
						...
						$.each(gv, function (k,v) {
							...
							grp_html += "<tr id='"+tr_id+"' style='height:20px;' onDblclick=\"FWMultiGrid.EditGridRow('"+system_module_name+"','"+table_id+"',"+tr_id+");\">";
						
							var row_obj=new Object;
							var group_int_code=v.group_int_code;
							//LOOP3
							...
							grp_html+="<td id='seq_"+tr_id+"' ><p style='width:30px;' class='td-txt' ></p></td>";
								//LOOP4
								$.each(grp_ele_arr, function (ek,ev) {	
									...
									if(data_type=='Reference Data')
									{
										if(populate_lov==1 && is_ref_module_code_lov==1)
										{
											...
											text_val=empty(text_val) ? "" : text_val;
											val=empty(row_val) ? "0" : row_val;
											grp_html+="<td "+td_display+"><p style='margin-top:0px; width:"+grid_width+"px;'  class='td-txt' title='"+text_val+"'> <input style='width:0px;' type='hidden' id='"+db_field+"_"+tr_id+"' value="+val+"><span id='txt_ref_schema_"+db_field+"_"+tr_id+"'>"+text_val+"</span></p></td>";
									...
									else if(data_type=='Date')
									{
										if(empty(val))
											...
											val=decodeURIComponent(val);
										grp_html+="<td id='"+db_field+"_"+tr_id+"' class='text-right' "+td_display+"><p  class='td-txt' style='margin-top:0px; width:"+grid_width+"px;' title='"+val+"'>"+val+"</p></td>";
									}
									else if(data_type=='Number')
									{
										if(val == 'null')
									...
									else if(data_type=='Attachment')
									{
										...
										if(empty(val))
											data_file_src="";
											grp_html+="<td id='"+db_field+"_"+tr_id+"' class='text-right' "+td_display+"><p  data-att-state='"+data_att_state+"' data-file-src='"+data_file_src+"' class='td-txt' style='width:"+grid_width+"px;'  data-file-name ='"+attch_name+"' >"+attch_name+"</p></td>";
									}
									else if(data_type=='Image')
									{
										var image_data="";
										...
										var image_tag = "<img style='max-width:100px; max-height:100px;' src='"+image_data+"' data-image-type='"+image_attr+"' data-image-name='"+img_file_name+"'> </img>";
										grp_html+="<td id='"+db_field+"_"+tr_id+"' class='text-center' "+td_display+"><p class='td-txt' style='margin-top:0px; width:"+grid_width+"px;'>"+image_tag+"</p></td>";
									}
									else if(data_type=='Checkbox')
									{
										var checked='';
										...
										grp_html+="<td id='"+db_field+"_"+tr_id+"' class='text-center' "+td_display+"><p  class='td-txt' style='margin-top:0px; width:"+grid_width+"px;'>"+checkbox_html+"</p></td>";
									}
									else if(data_type=='Large Text')
									{
									
										grp_html+="<td id='"+db_field+"_"+tr_id+"' class='text-left' "+td_display+"><p  class='td-txt' style='margin-top:0px; white-space:pre;width:"+grid_width+"px;' title='"+decodeURIComponent(val)+"'>"+decodeURIComponent(val)+"</p></td>";
									}
									else
									{
									
										grp_html+="<td id='"+db_field+"_"+tr_id+"' class='text-left' "+td_display+"><p  class='td-txt' style='margin-top:0px; width:"+grid_width+"px;' title='"+decodeURIComponent(val)+"'>"+decodeURIComponent(val)+"</p></td>";
									}
							...
							//ACTIONS
									var tr_actions="<span class='btn btn-sm blue tooltip-primary' tabindex=0  data-placement='bottom' onClick=\"FWMultiGrid.FormGridEditFields('"+system_module_name+"','"+table_id+"',"+tr_id+",'Edit')\" title='Edit' data-toggle='tooltip' data-original-title='Edit' style='padding: 0px 0px;'>"; 
									tr_actions+="<button type='button' class='btn btn-default btn-circle'> <i class='fa fa-edit' style='font-size:12px;'></i></button>";
									tr_actions+="</span>";
									...
									{
										//Add New Row                                                                                                            
										tr_actions+="&nbsp;<span class='btn btn-sm purple tooltip-primary'  tabindex=0 data-placement='bottom' onClick=\"FWMultiGrid.FormGridEditFields('"+system_module_name+"','"+table_id+"',"+tr_id+",'Add')\"  title='Add New' data-toggle='tooltip' data-original-title='Add New ' style='padding: 0px 0px;'>"; 
										tr_actions+="<button type='button' class='btn btn-info btn-circle'>  <i class='fa fa-plus'></i></button></span>";
									}
									if(parent_group==0)
									{
										...
										{
											//Edit Row in popup
											tr_actions+="&nbsp;<span class='btn btn-sm yellow tooltip-primary' tabindex=0  data-placement='bottom' onClick=\"FWMultiGrid.EditGroupRowinPopup('"+system_module_name+"','"+table_id+"',"+tr_id+",'RowPopup')\"  title='Edit in Popup' data-toggle='tooltip' data-original-title='Edit in Popup' style='padding: 0px 0px;'>"; 
											tr_actions+="<button type='button' class='btn btn-primary btn-circle'><i class='fa fa-external-link'></i> </button></span>";
											tr_actions+="<span id='third_level_obj_"+tr_id+"' style='display:none;'>"+row_obj['third_level_obj']+"</span>";
										}
									}
									...
									{
										//Delete Row
										tr_actions+="&nbsp;<span class='btn btn-sm red tooltip-primary'  tabindex=0 data-placement='bottom' onClick=\"FWMultiGrid.DeleteGroupRow('"+system_module_name+"','"+table_id+"',"+tr_id+")\"  title='Delete' data-toggle='tooltip' data-original-title='Delete' style='padding: 0px 0px;'>"; 
										tr_actions+="<button class='btn btn-danger btn-circle'><i class='fa fa-trash-o'></i></button></span>";
									}
									var child_group_html="";
									if(is_child_exists==1)
									{
										child_group_html+="		<span class='btn btn-sm dark tooltip-primary' tabindex=0  onclick=\"FWMultiGrid.OpenThirdLevelPopup('"+system_module_name+"','"+table_id+"','"+tr_id+"',"+group_int_code+");\" title='Third Level' data-placement='bottom' data-toggle='tooltip' data-original-title='Third Level' style='padding: 0px 0px;'>";
										child_group_html+="			<button type='button' class='btn btn-dribbble btn-circle'><i class='fa fa-sitemap'></i></button>";
										child_group_html+="		</span>";
									}
									tr_actions+="<input type='hidden' id='group_int_code_"+tr_id+"' value="+group_int_code+" />";
									...
									if(is_copy_row==1)
									{
										tr_actions+="<span class='btn btn-sm green tooltip-primary' tabindex=0  data-placement='bottom' onClick=\"FWMultiGrid.CopyRowData('"+system_module_name+"','"+table_id+"',"+tr_id+")\" title='Copy' data-toggle='tooltip' data-original-title='Copy' style='padding: 0px 0px;'>"; 
										tr_actions+="<i class='fa fa-copy' style='font-size:12px;'></i>";
										tr_actions+="</span>";
									}
									grp_html+="<td id='actions_"+tr_id+"' style='width:200px; text-align:center;'>"+tr_actions+"</td>";
								// removed <p style='width:120px; float:left;  margin: 0;'></p>for new fw,earlier was there	
						...
						$("#m_"+system_module_name+" #"+table_id +" tbody").append(grp_html);
						
						FWMultiGrid.FormGridEditFields(system_module_name,table_id,0,'Add');
						
						// FWMultiGrid.ArrangeRowSequence(system_module_name,table_id);
						// FWMultiGrid.CalculateGroupColumnTotal(system_module_name,table_id);
		...
		try {
			...
			if(!empty(grp_details))
			{
					...
					$.each(grp_details, function (k,v) {
						...
						$(input_id).append("<tr id='"+tr_id+"' style='height:20px;' onDblclick=\"FWMultiGrid.EditGridRow('"+system_module_name+"','"+group_table_name+"',"+tr_id+");\"></tr>");
						var row_obj=new Object;
	...
	CalculateGroupColumnTotal : function (system_module_name,group_table_name) {
			...
			if(group_footer=='Yes')
			{
				...
				$.each(grp_ele_arr, function (k,v) {
					...
					if(group_total=='Yes')
					{
						...
						{
							html_val = $(this).html();
							if(empty(html_val) || html_val == '')
								html_val = 0;
							total+= parseFloat(html_val); 
						});
		...
		try {
			...
			var footer_row=' <div class="clearfix"></div> ';
				footer_row+='	<div   style="margin:0;"'; 
				footer_row+='	class="pull-right"> '; 
				footer_row+="	<span class='btn btn-sm btn-info1'  onclick=\"FWMultiGrid.SaveThirdLevelGridData('"+system_module_name+"','"+group_table_name+"','"+tr_id+"');\" >Save <i class='fa fa-check'></i></span>"; 
				footer_row+="	<input type='hidden' id='pop_up_row_int_code' value='"+tr_id+"' /> <span class='btn btn-sm btn-danger'  onclick=\"$('#myModal').modal('hide')\" >Cancel <i class='fa fa-times'></i></span></div> "; 
			
			var jt_db=getCookie("jt_db");
			// $.get("html_modules/modules/"+jt_db+"/module_"+module_id+"_"+group_id+"_child.html", function(data) {
				// html = data+footer_row;
				// $.prompt(html, {
									// title:group_name,
									// loaded:function(event){
	...
	SaveThirdLevelGridData : function (system_module_name,group_table_name,tr_id) 
	{
		try 
		{
			var save_record = ModuleGridPreSaveValidation.PreSaveGridEditThirdLevel(system_module_name,group_table_name,tr_id);
			if(save_record)
			{
	...
	AssignThirdLevelDataToPopup : function (system_module_name,group_table_name,tr_id) {
		...
		try {
			...
			if(!empty(third_row_obj) && third_row_obj!='undefined')
			{
				...
				$.each(third_level_group_arr,function (k,v){
					...
					if(t_tax_structure_item_tax_table==group_table_name) return;// if item_tax_structure exists ignore
					var all_row_obj = "";
					if(!empty(third_row_obj) && third_row_obj.hasOwnProperty(m_group_table_name))
					all_row_obj=third_row_obj[m_group_table_name];
				
					 console.log(m_group_table_name);
