var AllModules=function()
{
	...
	this.close = function (system_module_name) {
	
		if(all_obj[system_module_name]['load_type']=='FW')
			home('home_dashboard');
		else
		{
			 var div_id="popup_module_dialog_"+all_obj[system_module_name]['g_str_module_id'];	
			...
			for($i = 0; $i < save_second_level.length; $i++)
					{
						...
						var cell_val= (trim($("#"+div_id+" #"+save_second_level[$i].system_name).val()));
							...
							{
								bootbox.alert(element_name+" is mandatory", function() {
							...
							else if(data_type=="Date")
							{
								...
								{
								bootbox.alert('Please Enter '+element_name+' Date format as dd/mm/yy');
							...
							else if(data_type=="Attachment" && typeof window.FileReader !== 'function' )
							{
								...
								bootbox.alert(element_name+" : The file API isn't supported on this browser yet. Update your browser to latest version.", function() {
										setTimeout(function(){div_sytem_name.focus();},2000);			
									});    
								return false;
	...
	this.saveModuleDetails=function(system_module_name)
	{
		  try
		  {
			...
			{
				alert("Hi "+getCookie("username")+", You dont have permission to Edit these record");
				return false;
			}
			var div_id="m_"+system_module_name;
			save_second_level=save_obj.g_user_schema_details_array.data[1];
				...
				if(!empty(save_obj.g_multi_group))
				{
					$.each(save_obj.g_multi_group, function (k,v) {
						...
						if(is_empty_rows_allowed==0)
						{
							...
							{
							alert("The "+group_name +" contains Empty Rows. Please Delete the same before you do the final save");	
									 ...
									 try
										{
											...
											if($responce.error_code == 0)
											{
												...
												all_obj[system_module_name]['geditinternalcode']=$responce.data.last_id;
												 ...
												 bootbox.alert($responce.data.msg);
	...
	this.GetSchemaModuleDetailsForUpdateNew=function (system_module_name)
	{
		...
		var display_module_name=save_obj['g_display_module_name'];
			...
			$.ajaxq("ajaxQueue",{
				...
				error:AjaxErrorMessage,
				data:"internal_code="+save_obj.geditinternalcode+"&table_name="+save_obj.g_system_module_table_name+"&schema_id="+save_obj.g_str_module_id,
				success: function populateSchemaModuleDetalisForUpdate(responce)
						{
							try
								...
								catch(err)
								{
									txt= err.message;
									App.unblockUI();
									bootbox.alert(txt+"<br> in GetSchemaModuleDetailsForUpdateNew");
								}
						}
	...
	this.AssginDataAfterEdit=function(system_module_name,responce)
	{
			...
			var next_pre_button="<span id='prev_button' style='float:left; margin-left:20px; margin-top: 10px; cursor:pointer;' title='Previous Record' onclick=\"javascript:all_obj.GetPrevNextModuleDetails('"+system_module_name+"','Previous',"+previous_int_code+");\" class=''> <i class='fa fa-arrow-circle-o-left' style='font-size: 31px; color:#0362FD; '></i></span>";
				next_pre_button+="<span id='next_button'style='float:left; margin-left:20px; margin-top: 10px;  cursor:pointer;' title='Next Record'  onclick=\"javascript:all_obj.GetPrevNextModuleDetails('"+system_module_name+"','Next',"+next_int_code+");\" class=''> <i class='fa fa fa-arrow-circle-o-right' style='font-size: 31px; color:#0362FD; '></i></span>";
	...
	this.GetGroupDetailsForSave = function(g_multi_group,system_module_name)
	{
		...
		$.each(g_multi_group, function (k,v) {
			...
			$.each($(table_id), function (rk,rv) {
				...
				var row_type = $(table_id+"#"+tr_id+" #actions #row_cancel").attr('data-row-type');
				...
				if(row_type!='New' && row_type!='Edit' && parent_group==0)
				{
					...
					$.each(elements, function (ek,ev) {
						...
						var tr_row_data=$(table_id+"#"+tr_id);
						...
						group_save_obj[group_name][i]['group_int_code']=$(table_id+"#"+tr_id+" #group_int_code_"+tr_id).val();
				 
						group_save_obj[group_name][i]['row_sequence']=$(table_id+"#"+tr_id+" #seq_"+tr_id+" p").html();;
						if(is_child_exists=="1")
						{
							
								var t_obj= decodeURIComponent($(table_id+"#"+tr_id+" #third_level_obj_"+tr_id).html());;
    ...
    this.InitModule=function()
	  {
		  ...
		  if(this.g_is_module_for_new_or_edit=="Add" || this.g_is_module_for_new_or_edit=="Clear")
			...
			var html_save="";
			 ...
			 if(this.load_type=="FW")
			 {
				 $("#module_menu_links").html(links_data);
				$("#page_main_tab_div").parent().after(footer_link);
			 }
			 else
			 {
				var popup_actions="#popup_module_dialog_"+this.g_str_module_id+" #popup_bc #module_menu_links";
				 $(popup_actions).html(links_data);
				 $("#m_"+this.g_system_module_table_name+" #page_main_tab_div").parent().after(footer_link);
...
function getModulesDetailsNew(system_module_name,module_name,module_id,is_module_for_new_or_edit,display_module_name,geditinternalcode,callback_func)
 { 
	if($('#page_main_div').is(":visible") && !$('#page_main_div #m_'+system_module_name).is(":visible") && is_module_for_new_or_edit=='Add' 
				&& display_module_name!='Display Employee Month Data'&& display_module_name!='Display All Employee Data')
	{
	
			this.getModulesDetailsNewForPopup(system_module_name,module_name,display_module_name,module_id,callback_func);
		
	}
	else
	{
		// console.log("#page_main_div #m_"+system_module_name);
			...
			$('#main_body').show();
			// App.blockUI({boxed:true,message:"Loading "+display_module_name});
			$('#page_main_div').load($path,function() 
			{	
			   ...
			   success: function GeneralUserSchemaDetailsResponse($responce)
						{
							 try  
							{
								...
								{
								 // $("#date").attr("data-date-start-date","-"+m_obj.ur_past_days+"d");
								 // $("#date").attr("data-date-end-date","+"+m_obj.ur_future_days+"d");
								}
